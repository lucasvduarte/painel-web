...
export const Validate = (isRequired: boolean) => {
    let validations = yup.object().shape({
        ...
        password: isRequired ? yup.string().trim().min(6, 'Senha muito pequena').max(60, 'Senha muito grande').required() : yup.string().trim().min(6).max(60),
        confirmPassword: isRequired ? yup.string().trim().min(6, 'Senha muito pequena').max(60, 'Senha muito grande').oneOf([yup.ref('password'), ''], 'As senhas não correspondem').required() : yup.string().trim().min(6).max(60).oneOf([yup.ref('password'), ''], 'As senhas não correspondem'),
